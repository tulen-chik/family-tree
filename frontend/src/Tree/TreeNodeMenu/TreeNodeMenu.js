  ...
  render() {
    ...
    return [
      ...
      <TreeNodeActions key="action_list" className="tree_actions">
        <TreeNodeHeader>Редактировать</TreeNodeHeader>
        <ul>
          <li><a href="#" className="edit" onClick={this.handleToggleEditable}>Редактировать</a></li>
          {!this.props.allowPartners || <li><a href="#" className="add_partner" onClick={this.handleAddPartner}>Добавить партнера</a></li>}
          {this.props.partners.map(partner =>
            <li key={'addchild_' + partner.id}><a href="#{partner.id}" className="add_child" onClick={(e) => {this.handleAddChild(e, partner.id)}}>Добавить ребенка с {partner.name}</a></li>
          )}
          <li><a href="#" className="member_delete" onClick={this.handleDelete}>Удалить</a></li>
        </ul>
      </TreeNodeActions>
    ];
  }
